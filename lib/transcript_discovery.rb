...
module TranscriptDiscovery
  ...
  def self.search(rss_item: {}, youtube_captions: nil, logger: nil)
    ...
    if (url = rss_item[:transcript_url])
      result = fetch_podcast_transcript(url, rss_item[:transcript_type])
      return result if result
    end

    # 2. <content:encoded> with substantial text
    if (content = rss_item[:content_encoded])
      result = check_content_encoded(content)
      return result if result
    end

    # 3. Episode web page
    if (link = rss_item[:link])
      result = scrape_episode_page(link)
      return result if result
    end
  ...
  def self.fetch_podcast_transcript(url, content_type = nil)
    log("Checking podcast:transcript → #{url}")
    ...
    log("Found transcript via podcast:transcript (#{word_count(text)} words)")
    { text: text, source: "podcast:transcript", quality: :high }
  rescue => e
    log("Failed to fetch podcast:transcript: #{e.message}")
    nil
  end

  # Check content:encoded for substantial text content.
  def self.check_content_encoded(html)
    text = strip_html(html).strip
    return nil if word_count(text) < MINIMUM_WORD_COUNT

    log("Found transcript in content:encoded (#{word_count(text)} words)")
    { text: text, source: "content:encoded", quality: :medium }
  end

  # Scrape an episode web page for transcript content.
  def self.scrape_episode_page(url)
    log("Checking episode page → #{url}")
    html = fetch_url(url)
    ...
    log("Found transcript on episode page (#{word_count(text)} words)")
    { text: text, source: "episode_page", quality: :high }
  rescue => e
    log("Failed to scrape episode page: #{e.message}")
    nil
  end
  ...
  class << self
    ...
    def fetch_url(url, limit: 3)
      ...
      log("HTTP fetch failed for #{url}: #{e.message}")
      nil
    end
