...
class SiteGenerator
  ...
  def generate_for_language(lang_code)
    ...
    index_html = render_layout(
      ...
      content: render_template("index.erb",
        ...
        podcast_description: @config.description ? linkify_markdown(escape_html(@config.description)) : nil,
  ...
  def render_layout(lang:, page_title:, css_path:, languages:, feed_url:, content:)
    ...
    footer_text = linkify_markdown(escape_html(@site_config.fetch(:footer, "Generated by podgen")))
