...
language_pipeline.rb:
  ...
  7b. Resolve episode cover via `resolve_episode_cover(title)`:
     Priority: --image PATH/thumb/last → per-feed image: none → --base-image PATH → per-feed base_image: PATH → ## Image base_image: PATH → YouTube thumbnail → nil
...
HTML comments (`<!-- ... -->`, including multi-line) are stripped from guidelines.md before parsing. Use comments to temporarily disable config entries (e.g. languages, sources).
...
podgen [flags] <command> <args>
  generate <podcast>   # Full pipeline (--lingq to upload to LingQ during generation)
                       ...
                       #   --image PATH|thumb|last  Per-episode cover ("thumb"=YouTube, "last"=~/Desktop screenshot)
