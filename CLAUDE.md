...
- YouTube publishing: `TimestampPersister` saves transcription segments to `_timestamps.json`, `SubtitleReconciler` fixes segment text using the reconciled transcript (Claude API call, preserves timestamps), `SubtitleGenerator` produces SRT, `VideoGenerator` creates MP4 (1fps, ultrafast), `YouTubeUploader` handles OAuth2 + Data API v3 (force-ssl scope). Old episodes without timestamps are retranscribed on demand. Unreconciled segments are auto-reconciled at publish time (backfill). Batch stops on `uploadLimitExceeded`/`quotaExceeded`
